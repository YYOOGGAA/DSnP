...
public:
   ...
   float ave() const;
   int _max(size_t&) const;
   int _min(size_t&) const;
   ...
   bool operator !() {if(!readin) return true; else return false; }
   // return this if JSON file has been read in; return NLL if not.
   operator void* () const { if(!readin) return NULL; else return (void*)this;}
