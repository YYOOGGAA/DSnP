...
DBAppendCmd::exec(const string& option)
{
   ...
   int num=0;
   if (!CmdExec::lexOptions(option,options,2)) return CMD_EXEC_ERROR; //可處理missing跟extra
   ...
   DBJsonElem elem(options[0],num);
   if (!dbjson.add(elem))
   {
     cerr<<"Error: Element with key \""<< elem.key()<<"\" already exists!!"<<endl;
     return CMD_EXEC_ERROR;
   }
...
DBReadCmd::exec(const string& option)
{
   ...
   for (size_t i = 0, n = options.size(); i < n; ++i) {
      if (myStrNCmp("-Replace", options[i], 2) == 0) {
         if (doReplace) return CmdExec::errorOption(CMD_OPT_EXTRA,options[i]);//前面已經replace
         doReplace = true;
      }
      else {
         if (fileName.size())
